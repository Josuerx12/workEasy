...
export type CompanyUserEntityValidatorProps = {
  name?: string;
  password?: string;
  email?: string;
  phone?: string;
...
export class CompanyUserEntityValidator {
  name?: string;
  password?: string;
  email?: string;
  phone?: string;
  ...
  constructor(props: CompanyUserEntityValidatorProps) {
  ...
  validate() {
    ...
    const companyUserValidated = companyUserSchema.safeParse(this);

    if (companyUserValidated.error) {
      throw companyUserValidated.error;
    }
    return true;
  }
}
